...
namespace Sdl.Web.Modules.Search.Models
{
    ...
    public abstract class SearchQuery : EntityModel
    {
        ...
        public Dictionary<string, string> QueryAgreegationParameters { get; set; }
        ...
        public bool IsFacetedSearchEnabled
        {
            get
            {
                string isFacetedSearchEnable = WebRequestContext.Localization.GetConfigValue("search.isFacetedSearchEnable");
                if (!string.IsNullOrEmpty(isFacetedSearchEnable))
                {
                    if(isFacetedSearchEnable.ToLower()=="yes")
                    {
                        return true;
                    }
                }
