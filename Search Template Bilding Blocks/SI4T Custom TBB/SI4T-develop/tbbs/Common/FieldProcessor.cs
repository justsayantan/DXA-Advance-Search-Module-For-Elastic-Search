        ...
        public virtual void ProcessFields(ItemFields fields, FieldProcessorSettings settings)
        {
            foreach (var field in fields)
            {
                ...
                if (field is ComponentLinkField)
                {
                    ...
                    if ((field as ComponentLinkField).Values != null && (field as ComponentLinkField).Values.Count >0)
                    {
                        foreach (Component cp in (field as ComponentLinkField).Values)
                        {
                            if (cp.Content != null)
                            {
                                ItemFields fieldsOfLinkComponent = new ItemFields(cp.Content, cp.Schema);
                                ProcessFields(fieldsOfLinkComponent, settings);
                            }
                        }
                    }
                }
